...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); // 向前兼容
	...
	_createClass(OneStroke, [{
		...
		value: function enter(index) {
			...
			this.lines.forEach(function (item) {
				...
				_this.addCoords({ x: x1, y: y1 }, { x: x2, y: y2 });
			});
		...
		value: function addCoords() {
			...
			coords.forEach(function (_ref2) {
				...
				for (var i = 0, len = _this2.coords.length; i < len; ++i) {
					if (_this2.coords[i].x === x && _this2.coords[i].y === y) {
						return false;
					}
				}
				_this2.coords.push({ x: x, y: y });
			});
		}
		...
		value: function drawBaseLines() {
			...
			this.baseLines = this.lines.map(function (_ref3) {
				...
				var line = new PIXI.Graphics().lineStyle(_this3.lineWidth, _this3.lineColor, 1).moveTo(x1, y1).lineTo(x2, y2).closePath();
				_this3.stage.addChild(line);
				return line;
			});
		}
		...
		value: function drawBaseVertexes() {
			...
			this.baseVertexes = this.coords.map(function (_ref4) {
				...
				var vertex = new PIXI.Graphics().beginFill(_this4.vertexColor, 1).drawCircle(0, 0, _this4.vertexRadius);

				vertex.set({ x: x, y: y });
				_this4.stage.addChild(vertex);
				return vertex;
			});
		}
		...
		value: function findValidCoords() {
			...
			this.coords.forEach(function (coord) {
				// 创建一个有效坐标数组 
				coord.validCoords = [];
				_this5.lines.forEach(function (_ref5) {
					var x1 = _ref5.x1,
					    y1 = _ref5.y1,
					    x2 = _ref5.x2,
					    y2 = _ref5.y2;
					...
					if (coord.x === x1 && coord.y === y1) {
						coord.validCoords.push(_this5.findCoord(x2, y2));
					}
					// 坐标是当前线段的终点
					else if (coord.x === x2 && coord.y === y2) {
							coord.validCoords.push(_this5.findCoord(x1, y1));
						}
				});
			});
		}
		...
		value: function pass() {
			...
			this.baseVertexes.forEach(function (vertex) {
				return TweenMax.to(vertex, 1, { scaleX: 2.5, scaleY: 2.5, alpha: 0 });
			});
			// 清空所有的底图线段
			this.baseLines.forEach(function (line) {
				return _this6.stage.removeChild(line);
			});
			// 敲落手绘线
			this.knockStrokes(function () {
				return _this6.event.dispatch("pass");
			});
		}
		// 下一关
...
},{"./lib/Events":326,"./lib/utils.es6":327,"babel-polyfill":1}]},{},[328]);
